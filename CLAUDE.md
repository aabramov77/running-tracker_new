...
**Каждая новая фича начинается с тикета в GitHub Issues.**

- Репозиторий: `aabramov77/running-tracker_new`.
- В начале работы над новой фичей — создать issue (`method: create`),
  получить авто-номер `#N`. Название issue = суть фичи; в теле — краткое
  описание/критерии.
- Коммиты по фиче ссылаются на номер в заголовке: `feat: #N краткое описание`
  (или `fix: #N …`). Ветка остаётся `Dev`.
- В теле PR указывать `Closes #N` — GitHub автоматически закроет issue при
  мерже в `main`.
- **План реализации коммитить в `docs/tickets/N-краткий-слаг.md`** (шапка со
  ссылками на issue/PR + контекст, изменения, верификация). В issue вставлять
  ссылку на этот файл — так план версионируется вместе с кодом.
  Примечание: GitHub-интеграция даёт только чтение issues, комментарии
  публикует пользователь вручную.
- Багфиксы и мелкие правки без отдельной фичи можно без тикета (по ситуации);
  тикеты — для новых фич.
