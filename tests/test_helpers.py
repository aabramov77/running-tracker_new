"""Чистые хелперы слоёв storage и llm_prompt (#36: раньше всё это жило в main.py).
...
def test_spm_none_when_no_cadence(storage_module):
    assert storage_module._spm({}) is None


def test_spm_doubles_base(storage_module):
    # FIT stores cadence per-leg; we return steps/min = (base + frac) * 2
    assert storage_module._spm({"avg_cadence": 90}) == 180


def test_spm_includes_fractional(storage_module):
    assert storage_module._spm({"avg_running_cadence": 88, "avg_fractional_cadence": 0.5}) == 177
...
def test_fmt_pace(storage_module, sec, expected):
    assert storage_module._fmt_pace(sec) == expected
...
def test_fmt_duration(storage_module, sec, expected):
    assert storage_module._fmt_duration(sec) == expected
...
def test_parse_llm_json_ok(storage_module, text, expected):
    assert storage_module.parse_llm_json(text) == expected


def test_parse_llm_json_raises_without_json(storage_module):
    with pytest.raises(Exception):
        storage_module.parse_llm_json("no json here at all")
...
def test_mask_key(storage_module, key, expected):
    assert storage_module.mask_key(key) == expected


# ── HR-drift / lap paces ──────────────────────────────────────────────────────

def test_hr_drift_none_when_empty(storage_module):
    assert storage_module.compute_hr_drift({}) is None


def test_hr_drift_none_when_too_few_laps(storage_module):
    assert storage_module.compute_hr_drift({"laps": [{"avg_hr": 150}, {"avg_hr": 155}]}) is None


def test_hr_drift_positive_when_hr_rises(storage_module):
    details = {"samples": {"hr": [120, 120, 140, 140]}}
    drift = storage_module.compute_hr_drift(details)
    assert drift is not None and drift > 0


def test_lap_paces_str_none_when_empty(storage_module):
    assert storage_module.lap_paces_str({}) is None


def test_lap_paces_str_truncates(storage_module):
    laps = [{"pace": "5:00"} for _ in range(20)]
    s = storage_module.lap_paces_str({"laps": laps}, limit=5)
    assert "(+15)" in s


# ── format_context_for_llm ────────────────────────────────────────────────────

def test_format_context_smoke(storage_module):
    ctx = {
        "last_runs": [
...
def test_format_context_includes_profile_block(storage_module):
...
def test_format_context_without_profile_has_no_block(storage_module):
    """Пустой профиль не должен добавлять в промпт пустых строк-заглушек."""
    ctx = {
        "profile": storage_module.empty_athlete_profile(),
        "profile_derived": storage_module.compute_athlete_derived(storage_module.empty_athlete_profile()),
        "personal_bests": [],
        "last_runs": [], "last_races": [], "current_week": None, "next_week": None,
...
def test_profile_block_keeps_zero_values(storage_module):
    ...
    assert "Опыт:" not in "\n".join(
        llm_prompt.format_profile_block(storage_module.empty_athlete_profile(), {}, []))


def test_profile_block_marks_estimated_hr_max(storage_module):
    """Оценка HRmax помечается — модель не должна принять её за измерение."""
    profile = {**storage_module.empty_athlete_profile(), "birth_date": "1986-01-01"}
    ...
    lines2 = "\n".join(llm_prompt.format_profile_block(
        measured, storage_module.compute_athlete_derived(measured, today=date(2026, 8, 16)), []))
...
def test_parse_synthetic_fit_summary_and_cadence(storage_module):
    ...
    parsed = storage_module.parse_fit_file(SYNTHETIC_FIT.read_bytes())
...
def test_parse_real_fit_if_present(storage_module):
    """Extra confidence on a real Garmin export when available locally."""
    parsed = storage_module.parse_fit_file(FIT_FIXTURE.read_bytes())
    summary = parsed["summary"]
    assert summary["dist_km"] > 0
    assert summary["avg_cadence"] is not None and summary["avg_cadence"] > 150
    assert len(parsed["laps"]) > 1
