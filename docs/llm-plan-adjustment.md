...
| # | Вопрос | Решение |
|---|---|---|
| 1 | UI настроек | Новый таб **«⚙ Настройки»** в навигации (рядом с «🏅 Старты») |
| 2 | Дефолтный провайдер | Не задавать. При первом заходе на «Корректировку» (или попытке вызвать LLM без конфига) — редирект/подсказка «Сначала задайте провайдера в Настройках». Форма «Настройки» открывается пустой. |
| 3 | Контекст LLM | **14** последних пробежек + последние **3** забега |
| 4 | Регенерация рекомендации | **Только по кнопке** (контроль стоимости) |
| 5 | Применение рекомендаций к плану | Фаза 1: **только текст**. Авто-патч плана через `POST /plan` НЕ делаем (даже как фаза 2 в этом плане — отдельная задача потом). |
| 6 | FIT samples (поминутные ряды) | **Хранить**: HR, темп, высота. Downsample до ~1 точки в 5 сек чтобы не разрастать. |
| 7 | Старые пробежки (без FIT) | **Оставить как есть** (`details_available: false`). Кнопку «Дозагрузить FIT» не делаем. |
| 8 | График HR/pace в модалке деталей | **Да, показывать** (Chart.js уже подключён). |

---

## Порядок реализации (предлагаемые фазы)

План объёмный — лучше дробить на самостоятельно работающие куски, по PR за фазу.

### Фаза 1 — Настройки LLM + базовые рекомендации (без FIT) ⭐ старт
Цель: дать рабочие LLM-советы на уже существующих данных (summary из `runs.json` + races + план).
- Бэк: `/config/llm` GET/POST + `/config/llm/test`; модуль `llm_clients` (3 провайдера через `httpx`); `/advise` GET/POST с компактным контекстом из текущего `runs.json`
- Фронт: новый таб «⚙ Настройки» (форма провайдер/модель/ключ + «Сохранить»/«Проверить»); кнопка «🤖 Получить рекомендации ИИ» в «Корректировке»; рендер ответа
- Деплой Cloud Run

**Результат:** работающие рекомендации, можно реально пользоваться. Без лап и графиков.

### Фаза 2 — FIT-загрузка + детали тренировки
- Бэк: `/runs/upload-fit` (multipart, `fitparse`), `/runs/{id}/details`, хранение `details.json` + сырого FIT, samples downsample 5 сек
- Фронт: input `.fit` в форме «+ Пробежка»; обогащение `runs.json`-записи флагом `details_available`
- Деплой Cloud Run

### Фаза 3 — обогащение контекста LLM лапами и HR-drift
- Бэк: `build_llm_context()` для последних 14 пробежек подгружает их `details.json`, считает HR-drift, формирует пер-лап выжимку
- Фронт: без изменений
- Деплой Cloud Run

### Фаза 4 — график HR/pace в модалке деталей тренировки
- Фронт: при открытии `showRunDetail()` для записи с `details_available` — рендер Chart.js линии HR и темпа во времени
- Бэк: без изменений
